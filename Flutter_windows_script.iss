...
OutputBaseFilename=FlutterWindowsService-x86_64-1.0.2+1-Installer
...
var
  ProgramDataPath, VersionFilePath, VersionJson: string;
begin
  ProgramDataPath := ExpandConstant('{commonappdata}\winserv_flutter');
  Log('ProgramData path: ' + ProgramDataPath);
  
  if not DirExists(ProgramDataPath) then begin
    Log('Directory does not exist. Creating...');
    CreateDir(ProgramDataPath);
  end else
    Log('Directory already exists.');

  VersionFilePath := ProgramDataPath + '\version.json';
  VersionJson := '{' + #13#10 +
                 '  "version": "' + '{#AppVer}' + '",' + #13#10 +
                 '  "buildNumber": "' + '{#BuildNum}' + '"' + #13#10 +
                 '}';

  if SaveStringToFile(VersionFilePath, VersionJson, False) then
    Log('version.json written successfully to: ' + VersionFilePath)
  else
    Log('Failed to write version.json.');
end;
